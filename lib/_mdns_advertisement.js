...
var MDNSService = require('_mdns_service').MDNSService
    , makeServiceType = require('_mdns_service_type').makeServiceType
    ;
...
function Advertisement(serviceType, port, options, callback) {
    ...
    if (txtRecord && typeof txtRecord === 'object' && !(txtRecord instanceof dns_sd.TXTRecordRef || Buffer.isBuffer(txtRecord))) {
        txtRecord = objectToTXTRecord(txtRecord);
    }

    function on_service_registered(serviceRef, flags, errorCode, name, serviceType, domain, context) {
        var error = dns_sd.buildException(errorCode);
        if (callback) {
            callback.call(self, error, {
                name: name, type: makeServiceType(serviceType), domain: domain, flags: flags
            }, context);
        }
        if (error) {
