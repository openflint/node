var dns_sd = require('_mdns_dns_sd')
    ...
    , util = require('util')
    , rst = require('_mdns_resolver_sequence_tasks')
    , st = require('_mdns_service_type')
    , MDNSService = require('_mdns_service').MDNSService
    ;
...
var Browser = exports.Browser = function Browser(serviceType, options) {
    ...
    function on_service_changed(sdRef, flags, ifaceIdx, errorCode, serviceName, serviceType, replyDomain, context) {
        ...
        if (errorCode == dns_sd.kDNSServiceErr_NoError) {
            ...
            var service = {
                interfaceIndex: ifaceIdx, type: st.makeServiceType(serviceType), replyDomain: replyDomain, flags: flags
            };
